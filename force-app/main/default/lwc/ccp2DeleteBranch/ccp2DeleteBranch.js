...
export default class Ccp2DeleteBranch extends LightningElement {
    ...
    get hasValidSelection() {
        ...
        // Check sections for branches and vehicles
        ...
        // Check sectionUser for branches and users
        ...
        // Both branches are selected but only vehicles are selected, not users
        if (hasBranchSelection && !hasValidVehicleSelection && !hasValidUserSelection) {
            return false;
        }

        // Either vehicles or users must be selected for any branch selection
    ...
    handleOutsideClick = (event) => {
        ...
        this.sections = this.sections.map(section => {
            ...
            console.log("outsideclick1",dataDropElement);
            const listsElement = this.template.querySelector(`.lists2`);
            console.log("outsideclick2",listsElement);
            if (
                dataDropElement &&
    ...
    handleOutsideClick2 = (event) => {
        console.log("outsideclick");
        this.sectionsUser = this.sectionsUser.map(section => {
            const dataDropElement = this.template.querySelector(`.Inputs1`);
            console.log("outsideclick1devil",dataDropElement);
            const listsElement = this.template.querySelector(`.lists2`);
            console.log("outsideclick2cevil",listsElement);
            if (
                dataDropElement &&
                !dataDropElement.contains(event.target) &&
                listsElement &&
                ...
                return { ...this.sectionUser, showconlist: false };
        ...
        if (selectedBranch) {
            ...
            if (isBranchAlreadySelected && previousSelectedValue !== selectedBranch.value) {
                ...
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error',
                        message: `The branch "${selectedBranch.label}" is already selected.`,
                        variant: 'error',
                    })
                );
            ...
            // Remove the previous selected value from the map
    ...
    handleCheckboxClick(event) {
        ...
        this.vehicleList = this.vehicleList.map(vehicle => {
            if (vehicleIds.includes(vehicle.value)) {
                return { ...vehicle, checked: true };
            } else {
                return { ...vehicle, checked: false };
            }
        });
