...
export default class Ccp2_VehicleDetails extends LightningElement {
  ...
  temDivClickHandler() {
    const temElm = this.template.querySelector(".file-input");
   
    if (temElm) {
      console.log("temElm", temElm)
     
      // Query the button inside the shadow root
      const uploadButton = temElm.querySelector("span");
      ...
      if (uploadButton) {
        ...
        console.error("Upload button not found in the shadow DOM.");
      }
    } else {
      console.error("elment not found in the DOM.");
    }
  ...
  handleUploadFinished(event) {
    ...
    uploadedFiles.forEach((file) => {
      ...
      const fileURL =
        "/sfc/servlet.shepherd/version/download/" + file.documentId;
